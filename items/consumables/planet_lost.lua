local consumInfo = {
    name = 'Lost Galaxy',
    set = "Planet",
    config = { hand_type = 'jojo_FlushFibonacci' },
}

consumInfo.loc_vars = function(self, info_queue, card)
    info_queue[#info_queue+1] = {key = "artistcredit", set = "Other", vars = { G.stands_mod_team.keku } }
...
consumInfo.in_pool = function(self, args)
    if next(SMODS.find_card("c_jojo_steel_tusk_4")) or next(SMODS.find_card('j_fnwk_plancks_jokestar')) then
        return (G.GAME and G.GAME.hands and G.GAME.jojo_FlushFibonacci.played > 0)
    end
end
